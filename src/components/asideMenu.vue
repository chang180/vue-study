<template>
  ...
  <div class="aside-menu">
    <div class="wraps">
      <label>
        縣市：<select v-model="currCity">
          <option v-for="c of cityList" :key="c"> {{ c }} </option>
          </select>
      </label>
      <label>
        行政區：<select v-model="currDistrict">
          <option v-for="d of districtList" :key="d.id"> {{ d.name }}</option>
          </select>
      </label>
    </div>
    ...
    <ul class="store-lists">
      ...
      v-for="s of filteredStores" :key="s.id" @click="$emit('triggerMarkerPopup', s.id)">
...
export default{
  ...
  methods:{
    keywordHighlight(val){
      return val.replace(new RegExp(this.keywords,'g'), `<span class="keyword">${this.keywords}</span>`)
    },
    openInfoBox(sid){
      this.showModal = true
      this.infoBoxSid = sid
